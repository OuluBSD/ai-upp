...
#endif
#if (defined flagWIN32 && defined flagSCREEN && defined flagOGL && !defined flagUWP)
struct ScrWinOgl {
	struct NativeSinkDevice;
	struct NativeContext;
	struct NativeEventsBase;
...
template <class Scr> struct ScreenSinkDeviceT : ScrSinkDevice {
	ScreenSinkDeviceT(VfsValue& n) : ScrSinkDevice(n) {}
...
template <class Scr> struct ScreenContextT : ScrContext {
	ScreenContextT(VfsValue& n) : ScrContext(n) {}
...
template <class Scr> struct ScreenEventsBaseT : ScrEventsBase {
	ScreenEventsBaseT(VfsValue& n) : ScrEventsBase(n) {}
...
#endif
#if (defined flagWIN32 && defined flagSCREEN && defined flagOGL && !defined flagUWP)
using WinOglSinkDevice = ScreenSinkDeviceT<ScrWinOgl>;
using WinOglContext = ScreenContextT<ScrWinOgl>;
using WinOglEventsBase = ScreenEventsBaseT<ScrWinOgl>;
#endif
