...
void Editor::SetBottom(int i)
{
	btabs.SetCursor(i);
	SyncBottom();
}

void Editor::ShowBottom(int i)
{
...
void Editor::ToggleBottom(int i)
{
	if(i == btabs.GetCursor() && IsBottomShown())
		HideBottom();
	else
...
bool Editor::IsBottomShown() const
{
	return editor_bottom.GetZoom() < 0;
}

void Editor::SwapBottom()
{
	if(IsBottomShown())
		HideBottom();
	else {
...
void Editor::HideBottom() {
	editor_bottom.Zoom(0);
	files.SetWantFocus();
	//SetBar();
}

void Editor::IdeSetBottom(Ctrl& ctrl)
{
...
void Editor::IdeRemoveBottom(Ctrl& ctrl)
{
	ctrl.Remove();
	bottomctrl = NULL;
	BTabs();
...
void Editor::IdeActivateBottom()
{
	SetBottom(BDEBUG);
}

void Editor::SetupError(ArrayCtrl& error, const char *s)
{
	error.AddColumn("File").SetDisplay(Single<Editor::FoundFileDisplay>());
	error.AddColumn("Line").SetDisplay(Single<Editor::TopAlignedDisplay>());
	error.AddColumn(s);
	error.AddIndex("INFO");
...
void Editor::ShowFound()
{
	if(ffound.IsCursor())
		GoToError(ffound);
}

void Editor::CopyError(bool all)
{
...
void Editor::CopyFound(bool all)
{
	String txt;
...
void Editor::GoToError(const ErrorInfo& f) {
	if(IsNull(f.file))
		return;
...
void Editor::GoToError(ArrayCtrl& a) {
	Value v = a.Get("INFO");
	if(v.Is<ErrorInfo>())
...
void Editor::FoundDisplay::Paint(Draw& w, const Rect& r, const Value& q, Color ink, Color paper, dword style) const
{
	String s = q;
	if(*s == '\1') {
...
void Editor::TopAlignedDisplay::Paint(Draw& w, const Rect& r, const Value& q, Color ink, Color paper, dword style) const
{
	w.DrawRect(r, paper);
...
void Editor::FoundFileDisplay::Paint(Draw& w, const Rect& r, const Value& q, Color ink, Color paper, dword style) const
{
	w.DrawRect(r, paper);
	DrawFileName(w, r, q, ink);
}

Size Editor::FoundFileDisplay::GetStdSize(const Value& q) const
{
	return GetDrawFileNameSize(q);
}

WString Editor::FormatErrorLine(String text, int& linecy)
{
	WString txt;
...
WString Editor::FormatErrorLineEP(String text, const char *ep, int& linecy)
{
	WString txt;
	int cx = error.HeaderObject().GetTabWidth(2) - error.HeaderTab(2).GetMargin() * 2;
	int x = 0;
