#pragma once
#ifndef _CtrlLib_TreeCtrl_h_
#define _CtrlLib_TreeCtrl_h_
...
// TreeCtrl - hierarchical tree control
...
public:
	...
	public:
		...
		Node& SetCtrl(Ctrl& _ctrl)                { ctrl = std::shared_ptr<Ctrl>(&_ctrl, [](Ctrl*){}); return *this; }
...
private:
	...
	std::unique_ptr<EditString> edit_string;
	Ctrl           *editor = NULL;
	int             edit_cursor = -1;
	...
	void   GatherOpened(int id, std::vector<int>& o);
	...
	void   RefreshSel();
	void   GatherSel(int id, std::vector<int>& sel) const;
	void   DoClick(Point p, dword flags, bool down, bool canedit);
	void   SyncInfo();
	void   SyncAfterSync(std::shared_ptr<Ctrl> restorefocus);
...
public:
	...
	int    GetChildCount(int id) const                         { return item[id].child.size(); }
	...
	std::vector<int> GetOpenIds() const;
	void   OpenIds(const std::vector<int>& ids);
	...
	std::vector<int>  InsertDrop(int parent, int ii, const TreeCtrl& src, PasteClip& x);
	std::vector<int>  InsertDrop(int parent, int ii, PasteClip& d);
	Image        GetDragSample();
...
public:
	virtual bool  Key(dword k, int cnt) override;
	virtual void  SetData(const Value& data) override;
	virtual Value GetData() const override;
