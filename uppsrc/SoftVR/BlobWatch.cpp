...
{
	struct blobwatch *bw = (struct blobwatch*)malloc(sizeof(*bw));
	...
	bw->debug = true;
	bw->el = (struct extent_line*)calloc(height, sizeof(*bw->el));
 ...
 */
static int process_scanline(uint8 *line, int width, int height, int y,
 ...
 */
static void process_frame(uint8 *lines, int width, int height,
 ...
 */
static int find_free_track(uint8 *tracked)
{
	int i;
		       ...
		       struct leds *leds, struct blobservation **output)
{
	...
	memset(ob->tracked, 0, sizeof(uint8) * MAX_BLOBS_PER_FRAME);
