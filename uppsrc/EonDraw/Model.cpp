...
void ModelComponent::Visit(Vis& v) {
	...
	if (e.IsLoading()) {
		...
		ModelPtr own_model = loader.GetModel();
...
bool ModelComponent::Arg(String key, Value value) {
...
void ModelComponent::SetModel(ModelPtr m) {
	model = m;
	loader.Clear();
	prefab_name.Clear();
	model_changed = true;
...
bool ModelComponent::Load(GfxDataState& state) {
	...
	if (gfx_hash == 0) {
		...
		if (model) {
			...
			auto& mdl = state.AddModel();
			gfx_hash = mdl.id;
			ModelCachePtr cache = GetEngine().GetMachine().Get<ModelCache>();
			if (cache) {
