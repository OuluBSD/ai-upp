...
void Mesh::Visit(Vis& v) {
	v VISV(vertices)
	  VIS_(indices)
	  VIS_(named_vertices)
	  VIS_(material)
	  VIS_(disable_textures)
	  VIS_(wireframe_only)
	  VIS_(use_quad);
}
...
void Mesh::UpdateBoundingBox() {
	if (!use_quad) {
	    ...
	    UPP::ivec3 tri_indices = GetTriangleIndices(0);
	    m_min = m_max = GetVertCoord(tri_indices[0]);
	    for(int i = 0; i < static_cast<int>(m_num_tri); i++) {
	        UPP::ivec3 tri_indices = GetTriangleIndices(i);
	        UPP::vec3 p0 = GetVertCoord(tri_indices[0]);
	        UPP::vec3 p1 = GetVertCoord(tri_indices[1]);
	        UPP::vec3 p2 = GetVertCoord(tri_indices[2]);
	        m_max = UPP::Max(m_max, p0);
	        m_min = UPP::Min(m_min, p0);
	        m_max = UPP::Max(m_max, p1);
	        m_min = UPP::Min(m_min, p1);
	        m_max = UPP::Max(m_max, p2);
	        m_min = UPP::Min(m_min, p2);
	    }
	}
	else {
		int m_num_quad = indices.GetCount() / 4;
	    UPP::ivec4 quad_indices = GetQuadIndices(0);
	    m_min = m_max = GetVertCoord(quad_indices[0]);
	    for(int i = 0; i < static_cast<int>(m_num_quad); i++) {
	        UPP::ivec4 quad_indices = GetQuadIndices(i);
	        UPP::vec3 p0 = GetVertCoord(quad_indices[0]);
	        UPP::vec3 p1 = GetVertCoord(quad_indices[1]);
	        UPP::vec3 p2 = GetVertCoord(quad_indices[2]);
	        UPP::vec3 p3 = GetVertCoord(quad_indices[3]);
	        m_max = UPP::Max(m_max, p0);
	        m_min = UPP::Min(m_min, p0);
	        m_max = UPP::Max(m_max, p1);
	        m_min = UPP::Min(m_min, p1);
	        m_max = UPP::Max(m_max, p2);
	        m_min = UPP::Min(m_min, p2);
	        m_max = UPP::Max(m_max, p3);
	        m_min = UPP::Min(m_min, p3);
	    }
	}
}
...
void Mesh::UpdateNormalsAndTangents() {
	...
	if (!use_quad) {
	    ...
	    for(int i = 0; i < static_cast<int>(m_num_vertex); i++) {
	        SetVertNormal( i, UPP::vec3(0));
	        SetVertTangent(i, UPP::vec3(0));
	    }
	    // is smooth
	    if(false) {
	        for(int i = 0; i < static_cast<int>(m_num_tri); i++) {
	            ...
	            UPP::vec3 p1 = GetVertCoord(tri_indices[1]);
	            UPP::vec3 p2 = GetVertCoord(tri_indices[2]);
	            UPP::vec3 e1 = safe_normalize(p1 - p0);
	            UPP::vec3 e2 = safe_normalize(p2 - p0);
	            UPP::vec3 n  = safe_normalize(UPP::Cross(e1, e2));
	            for(int j = 0; j < 3; j++) {
	                int vert_index = tri_indices[j];
	                SetVertNormal( vert_index, GetVertNormal(vert_index) + n);
	                SetVertTangent(vert_index, UPP::vec3(0));
	            }
	        }
...
void Mesh::TransformVertices(UPP::mat4 transform)
{
	//int m_num_vertex = vertices.GetCount();
    UPP::mat4 normal_transform = Transpose(Inverse(transform));
