...
6) Start `Vfs/ProgDB` as a line-oriented program database package for semantic agent tooling. The external planning tree lives under `D:\sblo\Dev-linux\Manager\2-plan\ai-upp\root\progdb`. Keep small function-body AST/statement nodes in compact ProgDB records rather than one `VfsValue` per node; use `VfsValueExt` only as an integration carrier when useful. Treat TOON as one useful sparse/debug/interchange format, not as the mandatory format for dense records. Meta-programming should use FOG thesis internals (`D:\sblo\Dev-linux\FOG\doc\thesis`) more than FOG syntax: potential/actual records, meta-type syntax predicates, staged parsing, composition, utility levels, usage dependency graphs, output channels, and source mapping. FOG code (`D:\sblo\Dev-linux\FOG\Fog`) is a secondary audit source for implemented mechanisms (`FogDerivable`, `FogScopeContext`, `FogMetaType`, `FogUtility`, file/usage/emit contexts), not proof that every thesis feature exists. The future ProgDB language may use Python-like indent/dedent scopes and syntactic type/name separation to avoid C++-style semantic parser coupling.
