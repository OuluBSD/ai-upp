...
- `PacketRouter::ConnectionInfo` now exposes the connection policy and port metadata, and `ScriptLoader::BuildNet()` forwards the serialized metadata (`policy=legacy-loop`, `credits=1`) so `TestRouterRuntimeFlowCounters` can assert the runtime connections honor the same hints that were stored with the router overlays.
...
- `share/eon/tests/00h_router_flow.eon` now includes `corefx.pipe` so the FxAudioCore effect emits through the router during the regression run, and `TestRouterRuntimeFlowCounters` treats `corefx.pipe` as another essential source when it matches connections via `PacketRouter::GetConnectionInfo()` so the FxAudioCore counter totals remain tied to the router diagnostics.
