...
public:
	...
	void NullCallback() {}
	void Quit() { auto tw = this->GetTopWindow(); if (tw) tw->Break(); }
	...
	void AddGUILayout()
	{
		...
		dlg.SetRect( this->GetWorkArea().CenterRect(300, 80) );
		EditString s;
		Button ok, cancel;
		ok.SetLabel(t_("Add"));
	...
	void SaveGUILayout()
	{
		this->SaveLayout(this->GetLayouts().GetKey( _GUILayouts.GetIndex() ));
		DeleteFile(ConfigFile("Layouts.bin"));
		StoreToFile(*this, ConfigFile("Layouts.bin"));
	}
	...
	void UpdateGUILayout()
	{
		...
		if (index < 0 || index >= this->GetLayouts().GetCount())
			return;
		this->LoadLayout(this->GetLayouts().GetKey( index ));
		_GUILayouts.GetButton(2).Enable((~_GUILayouts).ToString() != t_(" Default"));
	}

	void RemoveGUILayout()
	{
		int count = this->GetLayouts().GetCount();
		if (!count) return;

		this->DeleteLayout( this->GetLayouts().GetKey( _GUILayouts.GetIndex() ) );
		StoreToFile(*this, ConfigFile("Layouts.bin"));
		LoadGUILayouts();
		_GUILayouts <<= t_(" Default");
		this->LoadLayout(t_(" Default"));
		UpdateTools();
	}
	...
	void LoadGUILayouts(bool first = false)
	{
		...
		const ArrayMap<String, String>& p = this->GetLayouts();
		_GUILayouts.Clear();
		for (int i = 0; i < p.GetCount(); i++)
...
class DockableFormEdit : public FormEdit<DockWindow>
{
	
public:
	typedef DockableFormEdit CLASSNAME;
	DockableFormEdit();
...
class FormEditWindow : public FormEdit<TopWindow>
{
	Splitter vsplit, hsplit;
	ParentCtrl main;
	
public:
	typedef FormEditWindow CLASSNAME;
	FormEditWindow();
	
};

class FormEditCtrl : public FormEdit<ParentCtrl>
{
	Splitter vsplit, hsplit;
	ParentCtrl main;
	
public:
	typedef FormEditCtrl CLASSNAME;
	FormEditCtrl();
