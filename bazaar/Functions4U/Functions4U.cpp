...
bool LaunchFileCreateProcess(const char *file, const char *, const char *directory) {
	...
	if (!CreateProcessW(NULL, const_cast<wchar_t*>(wexec.ToStd().c_str()), NULL, NULL, FALSE, 0, NULL, ToSystemCharsetW(directory), &startInfo, &procInfo))  
		return false;
...
bool DirectoryMove(const char *dir, const char *newPlace) {
	...
	String wDir(dir), wNewPlace(newPlace);
    wDir.Cat(0);
    wNewPlace.Cat(0);
	std::wstring wsDir = wDir.ToWString().ToStd();
	std::wstring wsNewPlace = wNewPlace.ToWString().ToStd();
	
    SHFILEOPSTRUCTW fileOp = {};
	fileOp.hwnd = NULL;
    fileOp.wFunc = FO_MOVE;
    fileOp.pFrom = wsDir.c_str();
    fileOp.pTo = wsNewPlace.c_str();
...
bool FileToTrashBin(const char *path) {	
    ...
    String wpath(path);
    wpath.Cat(0);
	std::wstring wspath = wpath.ToWString().ToStd();
		
    SHFILEOPSTRUCTW fileOp = {};
    fileOp.hwnd = NULL;
    fileOp.wFunc = FO_DELETE;
    fileOp.pFrom = wspath.c_str();
    fileOp.pTo = NULL;
    fileOp.fFlags = FOF_ALLOWUNDO | FOF_NOCONFIRMATION | FOF_SILENT;
...
#else
	int fid = _wopen(fileName.ToWString().ToStd().c_str(), O_RDONLY|O_BINARY);
#endif
	if (fid < 0) 
		return String();
	const int size = 1024;
...
String LoadFile(const char *fileName, off_t from, size_t len)
{
	...
	int fid = open(fileName, O_RDONLY);
#else
	int fid = _wopen(String(fileName).ToWString().ToStd().c_str(), O_RDONLY|O_BINARY);
#endif
	if (fid < 0) 
		return String();
...
String GetExtExecutable(const String _ext)
{
	...
	WString fileW(file);
	WCHAR exe[1024];
	ret = FindExecutableW(fileW.ToStd().c_str(), NULL, exe);
...
String GetShellFolder2(int clsid) 
{
	...
	if(SHGetFolderPathW(NULL, clsid, NULL, //SHGFP_TYPE_CURRENT
											0, path) == S_OK) {
		return WString(std::wstring(path)).ToString();
	}
	return Null;
}
...
#else
	FILE *fp = _wfopen(String(file).ToWString().ToStd().c_str(), L"rb");
#endif
	if (fp != NULL) {
		int64 pos = 0;
		if (pos0 > 0) {
...
#else
	FILE *fp = _wfopen(fileName.ToWString().ToStd().c_str(), L"rb");
#endif
	if (fp != NULL) {
		int c;
		long i = 0;
