...
public:
    ...
    // Evaluates Omaha hand (4 hole, 5 board) choosing exactly 2 from hole and 3 from board.
    uint16_t evaluateOmaha(const std::array<unsigned, 4>& hole, const std::array<unsigned, 5>& board) const;

    // Evaluates 5-card Omaha hand (5 hole, 5 board) choosing exactly 2 from hole and 3 from board.
    uint16_t evaluateOmaha5(const std::array<unsigned, 5>& hole, const std::array<unsigned, 5>& board) const;

    // Evaluates a Hearts trick. Returns index of winner.
    int evaluateHeartsTrick(const std::vector<unsigned>& cards, int lead_suit) const;
