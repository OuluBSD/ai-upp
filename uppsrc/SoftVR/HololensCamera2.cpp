...
{
	OuvrtHoloLensCamera2 *self = (OuvrtHoloLensCamera2 *)transfer->user_data;
	int ret;
...
static int hololens_camera2_start(OuvrtDevice *dev)
{
	...
	self->transfer = (libusb_transfer**)calloc(self->num_transfers, sizeof(*self->transfer));
	if (!self->transfer)
		return -ENOMEM;
	...
	for (i = 0; i < self->num_transfers; i++) {
		...
		libusb_fill_bulk_transfer(self->transfer[i], devh,
					  bEndpointAddress, (uint8*)buf, BULK_TRANSFER_SIZE,
	...
	static struct DebugStreamDescription desc1 = {
		.width = HOLOLENS_CAMERA2_WIDTH,
		.height = HOLOLENS_CAMERA2_HEIGHT,
		.format = FORMAT_GRAY,
		.framerate = { 30, 1 },
	};
	self->debug1 = (struct debug_stream*)debug_stream_new(&desc1);

	static struct DebugStreamDescription desc2 = {
		.width = HOLOLENS_CAMERA2_WIDTH,
		.height = HOLOLENS_CAMERA2_HEIGHT,
		.format = FORMAT_GRAY,
		.framerate = { 60, 1 },
...
static void ouvrt_hololens_camera2_init(OuvrtHoloLensCamera2 *self)
{
	...
	self->frame = (uint8*)malloc(615706); /* 2*640*481 + 26 */
}
