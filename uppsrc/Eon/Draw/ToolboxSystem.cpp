...
void ToolComponent::AddTool(CustomToolComponentPtr cb) {
...
void ToolComponent::RemoveTool(CustomToolComponentPtr cb) {
	if (active_tool == cb) {
		if (tools.GetCount() > 1)
			SwitchNext();
		else
...
bool ToolboxSystemBase::Arg(String key, Value value) {
	if (key == "test.tool.changer")
		test_tool_changer = value.ToString() == "true";
	return true;
}

void ToolboxSystemBase::Attach(ToolComponentPtr tool) {
	ArrayFindAdd(tools, tool);
}

void ToolboxSystemBase::Detach(ToolComponentPtr tool) {
