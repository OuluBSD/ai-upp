            ...
            case 27:  // ESC key - command mode
                {
                    ...
                    if (command == "q") {
                        ...
                        if (out.IsOpen()) {
                            ...
                            clrtoeol();
                            attron(COLOR_PAIR(2) | A_BOLD);
                            printw("Error: Could not open file for writing: %s", vfsPath.ToStd().c_str());
                            attroff(COLOR_PAIR(2) | A_BOLD);
                            refresh();
                            getch(); // Wait for key press
                        }
...
// Fallback editor implementation when ncurses is not available
bool NcursesEditor::RunEditor(const String& vfsPath, const String& initialContent) {
    // Since ncurses is not available, implement a simple text-based editor
