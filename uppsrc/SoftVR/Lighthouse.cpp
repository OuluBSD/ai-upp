...
{
	struct lighthouse_ootx_report *report = (struct lighthouse_ootx_report *)(base->ootx + 2);
	...
	gravity[0] = report->gravity[0];
	gravity[1] = report->gravity[1];
	gravity[2] = report->gravity[2];
	vec3_normalize(&gravity);
	if (gravity[0] != base->gravity[0] ||
	    gravity[1] != base->gravity[1] ||
	    gravity[2] != base->gravity[2]) {
		base->gravity = gravity;
		g_print("Lighthouse Base %X: gravity: [ %9.6f %9.6f %9.6f ]\n",
			base->serial, gravity[0], gravity[1], gravity[2]);
	}
