...
bool WString::Is() const { 
    return !this->empty(); 
}

const wchar_t* WString::Begin() const { 
    return this->data(); 
}
const wchar_t* WString::End() const { 
    return this->data() + this->size(); 
}

int WString::operator[](int i) const { 
    return static_cast<unsigned int>(Base::operator[](static_cast<size_t>(i))); 
}

// Append
void WString::Cat(int c) { 
    this->push_back(static_cast<wchar_t>(c)); 
}
void WString::Cat(const wchar_t* s, int len) { 
    if(len > 0) this->append(s, static_cast<size_t>(len)); 
}
void WString::Cat(const wchar_t* s) { 
    if(s) this->append(s); 
}
void WString::Cat(const WString& s) { 
    this->append(s.data(), s.size()); 
}
void WString::Cat(int c, int count) { 
    if(count > 0) this->append(static_cast<size_t>(count), static_cast<wchar_t>(c)); 
}
...
// Insert
void WString::Insert(int pos, int c) { 
    this->insert(this->begin() + static_cast<ptrdiff_t>(pos), static_cast<wchar_t>(c)); 
}
void WString::Insert(int pos, const wchar_t* s, int count) { 
    this->insert(static_cast<size_t>(pos), s, static_cast<size_t>(count)); 
}
void WString::Insert(int pos, const WString& s) { 
...
int WString::Compare(const wchar_t* s) const { 
    return this->compare(s ? s : L""); 
}
bool WString::IsEqual(const WString& s) const { 
    return *this == s; 
}
bool WString::IsEqual(const wchar_t* s) const { 
    return this->compare(s ? s : L"") == 0; 
}

// Slices
WString WString::Mid(int pos, int length) const { 
    return WString(this->substr(static_cast<size_t>(pos), static_cast<size_t>(length))); 
}
WString WString::Mid(int pos) const { 
    return WString(this->substr(static_cast<size_t>(pos))); 
}
WString WString::Right(int count) const { 
    return count <= 0 ? WString() : WString(this->substr(this->size() - static_cast<size_t>(count))); 
}
WString WString::Left(int count) const { 
    return count <= 0 ? WString() : WString(this->substr(0, static_cast<size_t>(count))); 
}

// Find helpers
int WString::Find(int chr, int from) const {
    size_t p = this->find(static_cast<wchar_t>(chr), static_cast<size_t>(from));
    return p == Base::npos ? -1 : static_cast<int>(p);
}
int WString::ReverseFind(int chr, int from) const {
    if(from < 0) return -1;
    if(from >= GetLength()) from = GetLength() - 1;
    size_t p = this->rfind(static_cast<wchar_t>(chr), static_cast<size_t>(from));
    return p == Base::npos ? -1 : static_cast<int>(p);
}
int WString::ReverseFind(int chr) const {
    size_t p = this->rfind(static_cast<wchar_t>(chr));
    return p == Base::npos ? -1 : static_cast<int>(p);
}

int WString::Find(const wchar_t* s, int from) const {
    size_t p = this->find(s ? s : L"", static_cast<size_t>(from));
    return p == Base::npos ? -1 : static_cast<int>(p);
}
int WString::Find(const WString& s, int from) const {
    size_t p = this->find(s, static_cast<size_t>(from));
    return p == Base::npos ? -1 : static_cast<int>(p);
}
int WString::FindAfter(const wchar_t* s, int from) const {
    if(!s) return -1;
    int n = static_cast<int>(std::wcslen(s));
    int q = Find(s, from);
    return q < 0 ? -1 : q + n;
}
int WString::FindAfter(const WString& s, int from) const {
    int q = Find(s, from);
    return q < 0 ? -1 : q + static_cast<int>(s.size());
}

bool WString::StartsWith(const wchar_t* s, int len) const {
...
bool WString::StartsWith(const wchar_t* s) const {
...
std::string WString::WideToUtf8(const std::wstring& ws) {
#ifdef _WIN32
...
std::wstring WString::Utf8ToWide(const char* s, size_t len) {
#ifdef _WIN32
...
String WString::ToString() const { 
    return String(WideToUtf8(*this).c_str()); 
}
