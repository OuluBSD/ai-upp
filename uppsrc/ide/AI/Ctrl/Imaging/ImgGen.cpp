...
void ImageGenTool::ToolMenu(Bar& bar) {
	if (tabs.Get() == 0) {
		bar.Add(t_("Translate"), THISBACK1(Translate, 0)).Key(K_F5);
		bar.Add(t_("Upload"), THISBACK(Upload)).Key(K_F6);
		bar.Add(t_("Generate"), THISBACK(Generate)).Key(K_F7);
	}
	else {
		bar.Add(t_("Translate"), THISBACK1(Translate, 1)).Key(K_F5);
		bar.Add(t_("Upload"), THISBACK(Upload)).Key(K_F6);
		bar.Add(t_("Generate from editor"), THISBACK(GenerateFromEditor)).Key(K_F7);
		bar.Add(t_("Variate from editor"), THISBACK(VariateFromEditor)).Key(K_F8);
	}
}
