...
void Esc::SleepReleasing(int ms) {
	sleep = true;
...
void Esc::SleepInfiniteReleasing() {
	sleep = true;
	spinning_sleep = false;
	sleep_s = 0;
}
...
bool Esc::IsRunning() const {
	return !calls.IsEmpty();
}

bool Esc::IsSleepExit() const {
	return !fail && sleep && !spinning_sleep;
}

bool Esc::IsSleepFinished() const {
	return sleep && (sleep_s > 0 && ts.Seconds() >= sleep_s);
}

bool Esc::CheckSleepFinished() {
...
bool Esc::IsFailed() const {
	return fail;
}

void Esc::OnError(String s) {
	LOG("error: " << s);
	fail = true;
}
...
EscValue& Esc::VarGetAdd(const EscValue& key) {
...
IrValue::IrValue(const EscValue& v) {
	type = V_HIVAL;
	hv = v;
}
...
EscValue IrValue::AsEscValue(VmState& s) const {
	// ESC_VOID, ESC_DOUBLE, ESC_ARRAY, ESC_MAP, ESC_LAMBDA, ESC_INT64
	EscValue h;
	switch (type) {
		case V_VOID:	return EscValue();
		case V_REG:		return i32 >= 0 && i32 < REG_COUNT ? s.regs[i32] : EscValue();
		case V_CSTRING:	return str;
		case V_STRING:	return this->s;
		...
		case V_MAP:
			h.SetEmptyMap();
			for(int i = 0; i < map.GetCount(); i++)
				h.MapSet(map.GetKey(i), map[i].AsEscValue(s));
			return h;
		case V_ARRAY:
			h.SetEmptyArray();
			for(int i = 0; i < array.GetCount(); i++)
				h.ArrayAdd(array[i].AsEscValue(s));
			return h;
		case V_LABEL_INT:	return i32;
		case V_LABEL_STR:	return this->s;
		case V_VARSTACK:	return i32;
		default: return EscValue();
	}
}
...
void IrVM::FinishArgument() {
	...
	EscValue& v = parent->var.Get(return_argname);
	v = s->regs[0];
}

void IrVM::Yield() {
	run_ins_again = true;
	esc.SleepReleasing(1);
}
...
bool IrVM::Execute() {
	...
	while (IsRunning()) {
		...
		if (esc.sleep) {
			if (esc.spinning_sleep) {
				if (esc.ts.Seconds() >= esc.sleep_s) {
					esc.sleep = false;
					esc.spinning_sleep = false;
				}
				else {
					Sleep(1);
					continue;
...
void IrVM::Get(const SRVal& r, EscValue& v) {
	v = r.lval ? *r.lval : r.rval;
	GetSbs(r, v);
}

void IrVM::GetSbs(const SRVal& r, EscValue& v) {
	if(r.sbs.IsArray()) {
		const Vector<EscValue>& sbs = r.sbs.GetArray();
...
double IrVM::Number(const EscValue& a, const char *oper)
{
	if(a.IsNumber())
		return a.GetNumber();
...
int64 IrVM::Int(const EscValue& a, const char *oper)
{
	if(a.IsNumber())
		return a.GetInt64();
...
EscValue	IrVM::ReadVar(const IrValue& v) {
	EscValue r;
	r = v.AsEscValue(*s);
	return r;
}
...
void IrVM::WriteRegister(const IrValue& reg, const EscValue& v) {
	int i = -1;
...
void IrVM::Assign(const SRVal& val, const EscValue& src)
{
	if(!val.lval)
		OnError("l-value required");
...
void IrVM::Assign(EscValue& val, const Vector<EscValue>& sbs, int si, const EscValue& src)
{
	//LTIMING("Assign");
...
double IrVM::DoCompare(const EscValue& a, const EscValue& b, const char *op)
{
	//LTIMING("DoCompare");
	if(a.IsInt64() && b.IsInt64())
...
EscValue IrVM::MulArray(EscValue array, EscValue times)
{
	EscValue r;
	r.SetEmptyArray();
...
void IrVM::BeginExecutingLambda(const String& id, EscValue& lambda, SRVal& self, Array<SRVal>& arg) {
	if(!lambda.IsLambda()) {
...
int IrVM::InitLambdaExecution(EscLambda& l, IrVM& parent) {
	Array<SRVal>& arg = *parent.call_arg;
	SRVal& self = *parent.call_self;
	EscValue& sub_self = Self();
...
EscValue IrVM::ExecuteLambda(const String& id, EscValue& lambda, SRVal& self, Array<SRVal>& arg) {
	...
	Get(self, sub_self);
	for(int i = 0; i < l.arg.GetCount(); i++) {
		EscValue& v = sub.Var().GetAdd(l.arg[i]);
		if (i < arg.GetCount())
			Get(arg[i], v);
		else
