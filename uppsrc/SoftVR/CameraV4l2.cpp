...
static int ouvrt_camera_v4l2_start(OuvrtDevice *dev)
{
	...
	struct v4l2_streamparm parm = {
		...
		.parm.capture = {
			.timeperframe = {
				.numerator = 1,
				.denominator = (uint32)camera->framerate,
			}
		}
	};
	...
	if (1 /* DEBUG */) {
		...
		camera->sizeimage += sizeof(struct DebugAttachment);
	}
	...
	struct DebugStreamDescription desc = {
		.width = width,
		.height = height,
		.format = FORMAT_GRAY,
		.framerate = { (uint32)camera->framerate, 1 },
	};
 ...
 */
static void convert_yuyv_to_grayscale(uint8 *frame, int width, int height)
{
	uint8 *src, *dst;
	int x, y;
...
static void ouvrt_camera_v4l2_thread(OuvrtDevice *dev)
{
	...
	while (dev->active) {
		...
		if (v4l2->pixelformat == V4L2_PIX_FMT_YUYV)
			convert_yuyv_to_grayscale((uint8*)raw, width, height);
		...
		if (camera->tracker) {
			uint64 sof_time = buf.timestamp.tv_sec * 1000000000 +
					    buf.timestamp.tv_usec * 1000;

			ouvrt_tracker_process_frame(camera->tracker,
						    (uint8*)raw, width, height,
						    sof_time, &ob);
		}
...
{
	self->priv = (OuvrtCameraV4L2Private*)ouvrt_camera_v4l2_get_instance_private(self);
}
