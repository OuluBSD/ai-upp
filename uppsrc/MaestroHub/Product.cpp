...
void ProductPane::Load(const String& root_path) {
	root = root_path;
	RunbookManager rm(root);
	runbook_data = pick(rm.ListRunbooks());
	runbooks.Clear();
	for(const auto& rb : runbook_data)
		runbooks.Add(rb.title);
		
	WorkflowManager wm(root);
	Vector<String> list = wm.ListWorkflows();
	workflow_data.Clear();
	workflows.Clear();
	for(const auto& name : list) {
		WorkGraph& wg = workflow_data.Add();
		LoadFromJson(wg, wm.LoadWorkflow(name));
		workflows.Add(name);
	}
}
...
void ProductPane::OnRunbookSelect() {
	if(runbooks.IsCursor()) {
		...
		qtf << "------------------------------------------&";
		for(int i = 0; i < rb.steps.GetCount(); i++)
			qtf << "[* Step " << i+1 << ":] " << DeQtf(rb.steps[i].action) << "&";
...
void ProductPane::OnNodeRightClick(GraphLib::Node& n) {
	MenuBar::Execute([=, &n](Bar& bar) {
		bar.Add("Start Work", [=, &n] {
			OnNodeClick(n);
