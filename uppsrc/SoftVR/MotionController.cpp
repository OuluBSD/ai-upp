					     ...
					     G_GNUC_UNUSED const struct timespec *ts)
{
	uint8 buttons = buf[1];
	uint16 stick[2] = {
		(uint16)(buf[2] | ((buf[3] & 0xf) << 8)),
		(uint16)(((buf[3] & 0xf0) >> 4) | (buf[4] << 4)),
	};
	float joy[2] = {
		(float)(stick[0] * 2.0 / 4095 - 1.0),
		(float)(stick[1] * 2.0 / 4095 - 1.0),
	};
	...
	int32 accel[3] = {
		buf[9] | (buf[10] << 8) | ((int8)buf[11] << 16),
		buf[12] | (buf[13] << 8) | ((int8)buf[14] << 16),
		buf[15] | (buf[16] << 8) | ((int8)buf[17] << 16),
	};

	int32 gyro[3] = {
		buf[20] | (buf[21] << 8) | ((int8)buf[22] << 16),
		buf[23] | (buf[24] << 8) | ((int8)buf[25] << 16),
		buf[26] | (buf[27] << 8) | ((int8)buf[28] << 16),
	};

	uint32 time = buf[29] | (buf[30] << 8) | (buf[31] << 16) | (buf[32] << 24);
	int32 dt = time - self->last_timestamp;
	...
	struct imu_sample sample = {
		.time = raw.time * 1e-7,
		.acceleration = {
			accel[0] * STANDARD_GRAVITY / 506200.f,
			accel[2] * STANDARD_GRAVITY / 506200.f,
			-accel[1] * STANDARD_GRAVITY / 506200.f,
		},
		.angular_velocity = {
			gyro[0] * 1e-5f,
			gyro[2] * 1e-5f,
	...
	self->imu.pose.translation[0] = 0.0;
	self->imu.pose.translation[1] = 0.0;
	self->imu.pose.translation[2] = 0.0;
