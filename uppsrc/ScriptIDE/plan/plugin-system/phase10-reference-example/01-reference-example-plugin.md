...
The architecture needs validation. A reference plugin will prove that `.gamestate` files can be opened, `.xlay` files can be edited and loaded, and Python logic can drive a U++ UI layer successfully.

## Scope
- Defining the `CardGamePlugin` class implementing `IPlugin`.
- Defining the `CardGameView` (derived from `Ctrl` and `IDocumentHost`) for rendering `.xlay` at runtime.
- Defining the `CardGameEditor` for the `.xlay` FormEditor integration.
...
Create a high-level design of the plugin's internal class structure. Define the exact file layout of the Hearts example project (e.g., `main.py`, `table.xlay`, `hearts.gamestate`, `assets/`).
