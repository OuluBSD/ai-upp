				...
				if (classes.IsArray() && !HasArrayValue(classes, "class_actor")) {
					...
					if (states	  // object has a state?
						|| (state
							&& sdata
							&& sdata.GetInt() > 0)
						&& (!dependent_on // object has a valid dependent state?
							|| dependent_on("state") == obj("dependent_on_state"))
						&& !owner // object is not "owned"
						|| draw
                        || curr_anim) {
						if (states // object has a state?
							|| (state
