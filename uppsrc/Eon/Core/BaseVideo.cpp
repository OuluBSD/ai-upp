...
VideoGenBase::VideoGenBase(VfsValue& n) : Atom(n) {
	fmt.SetVideo(DevCls::CENTER, ColorSampleFD::U8_LE_ABC, VideoFormat::default_width, VideoFormat::default_height, 60, 1);
...
bool VideoGenBase::Send(RealtimeSourceConfig& cfg, PacketValue& out, int src_ch) {
	...
	if (packet_router && !router_source_ports.IsEmpty() && current_fmt.IsValid()) {
		...
		if (credits <= 0) {
			RTLOG("VideoGenBase::Send: credit request denied for src_ch=" << src_ch);
			return false;
		}
