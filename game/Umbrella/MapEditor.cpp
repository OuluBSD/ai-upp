...
void MapEditorApp::SetupToolBar() {
	// Helper lambda to pad icons to 24x24
	auto PadIcon = [](const Image& img) -> Image {
		Size sz = img.GetSize();
		if(sz.cx >= 24 && sz.cy >= 24)
			return img;

		ImageDraw iw(24, 24);
		iw.DrawRect(0, 0, 24, 24, SColorFace());
		int x = (24 - sz.cx) / 2;
		int y = (24 - sz.cy) / 2;
