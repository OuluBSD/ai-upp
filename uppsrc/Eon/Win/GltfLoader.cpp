...
namespace Gltf
{
        ...
        std::map<int, std::shared_ptr<Pbr::Material>> materialMap;
        {
            ...
            for (const auto& primitiveBuilderPair : primitiveBuilderMap)
            {
                ...
                if (materialIndex == -1) // No material was referenced. Make up a material for it.
                {
                    ...
                    pbrMaterial = Pbr::Material::CreateFlat(pbrResources, DirectX::Colors::Gray, 0.5f);
                }
                else
                {
        ...
        CXMMATRIX rootTransform)
    {
        ...
        if (!loader.LoadBinaryFromMemory(&gltfModel, &errorMessage, &warningMessage,
                                         reinterpret_cast<const unsigned char*>(buffer), bufferBytes, "."))
        {
