...
void SynCoreSynth::Instrument_Visit(NativeInstrument& dev, AtomBase&, Visitor& vis) {
...
bool SynCoreSynth::Instrument_Send(NativeInstrument& dev, AtomBase& a, RealtimeSourceConfig& cfg, PacketValue& out, int src_ch) {
	...
	if (fmt.IsAudio()) {
		...
		if (a.packet_router && !a.router_source_ports.IsEmpty() && fmt.IsValid()) {
			...
			if (credits <= 0) {
				RTLOG("SynCoreSynth::Instrument_Send: credit request denied for src_ch=" << src_ch);
				return false;
			}
...
bool SynCoreSynth::Instrument_IsDebugSoundEnabled(const NativeInstrument& dev, const AtomBase&) {
	return dev.debug_sound_enabled;
}

String SynCoreSynth::Instrument_GetDebugSoundOutput(const NativeInstrument& dev, const AtomBase&) {
	return dev.debug_sound_output;
}

int SynCoreSynth::Instrument_GetDebugSoundSeed(const NativeInstrument& dev, const AtomBase&) {
	return dev.debug_sound_seed;
}

bool SynCoreSynth::Instrument_IsDebugPrintEnabled(const NativeInstrument& dev, const AtomBase&) {
