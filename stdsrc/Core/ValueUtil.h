#pragma once
#ifndef _Core_ValueUtil_h_
#define _Core_ValueUtil_h_
...
template <class T>
inline const T& Nvl(const T& a, const T& b) { 
    return IsNull(a) ? b : a; 
}
...
inline int StdValueCompare(const Value& a, const Value& b, const LanguageInfo& f) {
    // TODO: Implement proper comparison with language-specific rules
    return StdValueCompare(a, b);
}

inline int StdValueCompare(const Value& a, const Value& b, int language) {
    // TODO: Implement proper comparison with language-specific rules
    return StdValueCompare(a, b);
}

inline int StdValueCompare(const Value& a, const Value& b) {
    // A basic comparison implementation - this would need to be expanded for complete functionality
    if (a.GetType() != b.GetType()) {
        return (int)a.GetType() - (int)b.GetType();  // Different types, sort by type
    }
    
    // For now, convert to string and compare - this is just a placeholder
    return strcmp(~AsString(a), ~AsString(b));
}

inline int StdValueCompareDesc(const Value& a, const Value& b, const LanguageInfo& f) {
    return -StdValueCompare(a, b, f);
}

inline int StdValueCompareDesc(const Value& a, const Value& b, int language) {
    return -StdValueCompare(a, b, language);
}

inline int StdValueCompareDesc(const Value& a, const Value& b) {
    return -StdValueCompare(a, b);
}

inline StdValuePairOrder::StdValuePairOrder(int l) : language(l) {
}

inline bool StdValuePairOrder::operator()(const Value& keya, const Value& valuea, const Value& keyb, const Value& valueb) const {
    int cmp = StdValueCompare(keya, keyb, language);
    if (cmp == 0) {
        return StdValueCompare(valuea, valueb, language) < 0;
    }
    return cmp < 0;
}

inline bool FnValuePairOrder::operator()(const Value& keya, const Value& valuea, const Value& keyb, const Value& valueb) const {
    return (*fn)(keya, valuea, keyb, valueb) < 0;
}

inline int CompareStrings(const Value& a, const Value& b, const LanguageInfo& f) {
    // TODO: Implement proper string comparison with language-specific rules
    String sa = AsString(a);
    String sb = AsString(b);
    return strcmp(~sa, ~sb);
}

// Implementations for Ref class constructors
template<typename T>
inline Ref::Ref(T& ref) : m(&Single< StdRef<T> >()), ptr(&ref) {}

inline Ref::Ref(String& s) : m(&Single< StdRef<String> >()), ptr(&s) {}
inline Ref::Ref(WString& s) : m(&Single< StdRef<WString> >()), ptr(&s) {}
inline Ref::Ref(int& i) : m(&Single< StdRef<int> >()), ptr(&i) {}
inline Ref::Ref(int64& i) : m(&Single< StdRef<int64> >()), ptr(&i) {}
inline Ref::Ref(double& d) : m(&Single< StdRef<double> >()), ptr(&d) {}
inline Ref::Ref(float& f) : m(&Single< StdRef<float> >()), ptr(&f) {}
inline Ref::Ref(bool& b) : m(&Single< StdRef<bool> >()), ptr(&b) {}
inline Ref::Ref(Date& d) : m(&Single< StdRef<Date> >()), ptr(&d) {}
inline Ref::Ref(Time& t) : m(&Single< StdRef<Time> >()), ptr(&t) {}
inline Ref::Ref(Value& v) : m(&Single< StdRef<Value> >()), ptr(&v) {}
