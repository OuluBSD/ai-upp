#pragma once
#ifndef _CtrlLib_RichText_h_
#define _CtrlLib_RichText_h_
...
// RichTextView - rich text viewing control
...
private:
	...
	void Print();
	Pos  GetPos();
	std::vector<int> ScPositions(const std::vector<int>& p);
	void Ids(int pid, std::vector<int>& r);
	std::vector<int> Ids();
...
public:
	...
	bool Up(const std::vector<int>& poslist);
	bool Down(const std::vector<int>& poslist);
...
void Print(Draw& w, const RichText& text, const Rect& page, const std::vector<int>& pagelist);
