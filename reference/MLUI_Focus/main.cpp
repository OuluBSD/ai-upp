...
FocusRouteDef& RegisterFocusRoute(const String& id, const String& path, const FocusPageDef& page, const String& summary, bool is_default) {
...
FocusLinkDef& RegisterFocusLink(const String& id, const FocusRouteDef& from_route, const FocusRouteDef& to_route, const String& label, const String& condition) {
	FocusLinkDef& l = Upsert(LinkRegistry(), id);
	l.from_route = from_route.id;
	l.to_route = to_route.id;
	l.label = label;
...
INITBLOCK {
	...
	FocusPageDef& issue_board = RegisterFocusPage("issue_board", "Issue Board", "Compact board view with triage actions")
		.Route("issues_list")
		.Component("board_table")
		.Action("select_issue")
		.Action("set_status");

	FocusPageDef& issue_editor = RegisterFocusPage("issue_editor", "Issue Editor", "Single issue editing view")
		.Route("issue_detail")
		.Component("editor_form")
		.Action("update_issue")
		.Action("close_issue");

	FocusRouteDef& issues_list = RegisterFocusRoute("issues_list", "/issues", issue_board, "Open issue list", true);
	FocusRouteDef& issue_detail = RegisterFocusRoute("issue_detail", "/issues/:key", issue_editor, "Open issue detail");
...
MLUIFocusReferenceApp::MLUIFocusReferenceApp()
{
	...
	editor.Add(issue_key.LeftPos(445, 130).TopPos(10, 20));

	editor.Add(title_label.LeftPos(10, 50).TopPos(40, 20));
	editor.Add(issue_title.HSizePos(90, 10).TopPos(40, 20));
	...
	editor.Add(notes.HSizePos(10, 10).VSizePos(100, 10));
	...
	MLUI::PushFocusHistory("", "issues_list", "open_app");
}
...
void MLUIFocusReferenceApp::OnBoardSelection()
{
	String prev_route;
	{
		MLUI::FocusStateDef& st = MLUI::GetFocusState("issue_editor_state");
	...
	MLUI::PushFocusHistory(prev_route, "issue_detail", "select_issue");
	MLUI::PushFocusDevtools("ui", "info", String().Cat() << "Selected issue " << issue_key.GetData());
}

void MLUIFocusReferenceApp::OnStatusChange()
{
	String st = status.GetData();
	MLUI::GetFocusState("issue_editor_state")
		.Set("status", st)
		.Set("dirty", true);
	MLUI::PushFocusDiff("editor_form", "replace", "/status", st);
	MLUI::PushFocusDevtools("action", "debug", String().Cat() << "Status changed to " << st);

	bool can_close = (st == "Done");
	MLUI::SetFocusPermission("close_issue", can_close, can_close ? String() : String("Issue status must be Done"));
}
...
void MLUIFocusReferenceApp::OnAssigneeChange()
{
	...
	MLUI::GetFocusState("issue_editor_state")
		.Set("assignee", own)
		.Set("dirty", true);
	MLUI::PushFocusDiff("editor_form", "replace", "/assignee", own);
	MLUI::PushFocusDevtools("action", "debug", String().Cat() << "Assignee changed to " << own);
}
...
void MLUIFocusReferenceApp::RefreshEditorFromSelection()
{
	...
	if(row < 0) {
		...
		MLUI::GetFocusState("issue_board_state").Set("selected_key", "");
		MLUI::GetFocusState("issue_editor_state")
			.Set("route", "/issues")
			.Set("selected_issue", "")
			.Set("dirty", false);
		MLUI::SetFocusPermission("close_issue", false, "Select issue first");
		return;
	}
	...
	MLUI::GetFocusState("issue_board_state").Set("selected_key", key);
	MLUI::GetFocusState("issue_editor_state")
	...
	bool can_close = (st == "Done");
	MLUI::SetFocusPermission("close_issue", can_close, can_close ? String() : String("Issue status must be Done"));
	MLUI::PushFocusDiff("board_table", "replace", "/selected_key", key);
}
