...
void PluginManager::LoadPlugins()
{
	...
	for(int i = 0; i < factories.GetCount(); i++) {
		...
		if(q >= 0)
			plugins[q].plugin = pick(p);
		else
			plugins.Add(id).plugin = pick(p);
	}

	// ScriptIDE must prefer GUI-capable plugins over headless ScriptCommon variants
	// when they intentionally share the same plugin id.
