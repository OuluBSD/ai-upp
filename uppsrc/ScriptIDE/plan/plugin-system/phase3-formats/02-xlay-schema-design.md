# Task: `.xlay` Schema Design

## Goal
Design the YAML schema for `.xlay` files, which define 2D scenes and table layouts for card games.

## Background / Rationale
To separate visual layout from game logic, we need a declarative layout format (`.xlay`). This format must be understandable by both a visual editor (FormEditor-derived) and a runtime renderer that the Python VM will interact with.

## Scope
- Defining the YAML schema for 2D scene/table layout.
...
2. Look at how U++ `lay` (layout) files are structured internally (though `.xlay` will be YAML).
3. Draft an initial YAML schema accommodating zones, anchors, and sprites.

## Affected Subsystems
- Plugin System / Layout Engine

## Implementation Direction
Produce a documented YAML schema and an example `.xlay` file representing a basic 4-player card table.
