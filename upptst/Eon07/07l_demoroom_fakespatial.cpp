...
void Run07lDemoroomFakespatial(Engine& eng, int method) {
	...
	case 1:
	case 2:
		LOG(Format("warning: Run07lDemoroomFakespatial: method %d not implemented yet", method));
	case 0:
		sys->PostLoadFile(ShareDirFile("eon/tests/07l_demoroom_fakespatial.eon"));
		break;
	default:
		throw Exc(Format("Run07lDemoroomFakespatial: unknown method %d", method));
	}
}
