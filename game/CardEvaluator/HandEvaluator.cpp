...
uint16_t HandEvaluator::LOOKUP[86547];
...
void HandEvaluator::outputTableStats(const char* name, const void* p, size_t elementSize, size_t count)
{
    ...
    for (size_t i = 0; i < elementSize * count; i += 64) {
        ...
        for (size_t j = 0; j < 64 && i + j < elementSize * count; j += elementSize) {
            if (memcmp((const char*)p + i + j, dummy, elementSize)) {
                ++usedElements;
                used = true;
            }
        }
...
uint16_t HandEvaluator::evaluateOmaha(const std::array<unsigned, 4>& hole, const std::array<unsigned, 5>& board) const
{
	uint16_t maxScore = 0;
	
	// Pre-build hole pairs (6 combinations)
	static const int holeIdx[6][2] = { {0,1}, {0,2}, {0,3}, {1,2}, {1,3}, {2,3} };
	Hand holeHands[6];
	for (int i = 0; i < 6; ++i)
		holeHands[i] = Hand(hole[holeIdx[i][0]]) + Hand(hole[holeIdx[i][1]]);
		
	// Pre-build board trios (10 combinations)
	static const int boardIdx[10][3] = {
		{0,1,2}, {0,1,3}, {0,1,4}, {0,2,3}, {0,2,4},
		{0,3,4}, {1,2,3}, {1,2,4}, {1,3,4}, {2,3,4}
	};
	Hand boardHands[10];
	for (int i = 0; i < 10; ++i)
		boardHands[i] = Hand(board[boardIdx[i][0]]) + Hand(board[boardIdx[i][1]]) + Hand(board[boardIdx[i][2]]);
		
	// Evaluate all 60 combinations
	for (int i = 0; i < 6; ++i) {
		for (int j = 0; j < 10; ++j) {
			uint16_t score = evaluate(holeHands[i] + boardHands[j]);
...
uint16_t HandEvaluator::evaluateOmaha5(const std::array<unsigned, 5>& hole, const std::array<unsigned, 5>& board) const
{
	uint16_t maxScore = 0;
	
	// Pre-build hole pairs (10 combinations for 5 cards)
	static const int holeIdx[10][2] = { 
		{0,1}, {0,2}, {0,3}, {0,4}, 
		{1,2}, {1,3}, {1,4}, 
		{2,3}, {2,4}, 
		{3,4} 
...
int HandEvaluator::evaluateHeartsTrick(const std::vector<unsigned>& cards, int lead_suit) const
{
	int winner = 0;
	int best_rank = -1;
	for (int i = 0; i < (int)cards.size(); i++) {
		int suit = cards[i] & 3;
		int rank = cards[i] >> 2;
		if (suit == lead_suit && rank > best_rank) {
			best_rank = rank;
			winner = i;
		}
	}
