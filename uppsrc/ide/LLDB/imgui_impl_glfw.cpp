...
//  Disable with 'io.ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange' (note: the resizing
//  cursors requires GLFW 3.4+). [X] Platform: Keyboard arrays indexed using GLFW_KEY_* codes,
...
// example of using this. If you are new to dear imgui, read examples/README.txt and read the
// documentation at the top of imgui.cpp. https://github.com/ocornut/imgui

// CHANGELOG
// (minor and older changes stripped away, please see git history for details)
//  2020-01-17: Inputs: Disable error callback while assigning mouse cursors because some X11
//  setup don't have them and it generates errors. 2019-12-05: Inputs: Added support for new
//  mouse cursors added in GLFW 3.4+ (resizing cursors, not allowed cursor). 2019-10-18: Misc:
//  Previously installed user callbacks are now restored on shutdown. 2019-07-21: Inputs: Added
//  mapping for ImGuiKey_KeyPadEnter. 2019-05-11: Inputs: Don't filter value from character
//  callback before calling AddInputCharacter(). 2019-03-12: Misc: Preserve
//  DisplayFramebufferScale when main window is minimized. 2018-11-30: Misc: Setting up
//  io.BackendPlatformName so it can be displayed in the About Window. 2018-11-07: Inputs: When
//  installing our GLFW callbacks, we save user's previously installed ones - if any - and
//  chain call them. 2018-08-01: Inputs: Workaround for Emscripten which doesn't seem to handle
//  focus related calls. 2018-06-29: Inputs: Added support for the ImGuiMouseCursor_Hand
...
//  2018-02-20: Inputs: Added support for mouse cursors (ImGui::GetMouseCursor() value, passed
//  to glfwSetCursor()). 2018-02-06: Misc: Removed call to ImGui::Shutdown() which is not
//  available from 1.60 WIP, user needs to call CreateContext/DestroyContext themselves.
//  2018-02-06: Inputs: Added mapping for ImGuiKey_Space.
//  2018-01-25: Inputs: Added gamepad support if ImGuiConfigFlags_NavEnableGamepad is set.
//  2018-01-25: Inputs: Honoring the io.WantSetMousePos by repositioning the mouse (when using
//  navigation and ImGuiConfigFlags_NavMoveMouse is set). 2018-01-20: Inputs: Added Horizontal
//  Mouse Wheel support. 2018-01-18: Inputs: Added mapping for ImGuiKey_Insert. 2017-08-25:
//  Inputs: MousePos set to -FLT_MAX,-FLT_MAX when mouse is unavailable/missing (instead of
//  -1,-1). 2016-10-15: Misc: Added a void* user_data parameter to Clipboard function handlers.
...
#ifdef GLFW_RESIZE_NESW_CURSOR // let's be nice to people who pulled GLFW between 2019-04-16
                               // (3.4 define) and 2019-11-29 (cursors defines) // FIXME:
...
void ImGui_ImplGlfw_KeyCallback(GLFWwindow* window, int key, int scancode, int action,
                                int mods)
{
                                ...
                                GlfwClientApi client_api)
{
    ...
    // cursor doesn't exist, GLFW will emit an error which will often be printed by the app, so
    // we temporarily disable error reporting. Missing cursors will return NULL and our
