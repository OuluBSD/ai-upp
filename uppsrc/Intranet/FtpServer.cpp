...
void FTPServer::Listener()
{
	...
	while(running && !Thread::IsShutdownThreads()) {
		...
		if(sock->Accept(server)) {
			lock.Enter();
			client_list.Add(id);
			Thread::Start(THISBACK2(ServeClient, id, sock.Detach()));
			lock.Leave();
		}
		...
		for(int i = 0; i < client_list.GetCount(); i++) {
			if (client_list[i].IsStopped())
				rmlist << i;
		}
		if (!rmlist.IsEmpty()) {
			lock.Enter();
			client_list.Remove(rmlist);
			lock.Leave();
		}
	}
	stopped = true;
...
void FTPServer::ServeClient(int id, TcpSocket* sockp)
{
	One<TcpSocket> sock = sockp;
...
String FTPClientConnection::AppendCwdSys(String s) { return GetSysDir(AppendCwd(s)); }
	...
	while(running && sock.IsOpen()) {
		...
		if(command == "USER") {
			...
			while(ff) {
				...
				unsigned long length = (unsigned long)ff.GetLength();
				String filename = ff.GetName();

				sprintf(line, "%s   1 %-10s %-10s %10lu Jan  1  1980\t%s\r\n", permstr,
				        username, username, length, filename.Begin());
			...
			String path = ChangeDirectorySys(arg);
			FindFile ff(path);
			String size = ff ? IntStr64(ff.GetLength()) : "0";
			...
			// YYYYMMDDhhmmss
			String time = Format("%04d%02d%02d%02d%02d%02d", (int)t.year, (int)t.month,
