...
void FtpBrowser::Connect()
{
	...
	if(!browser.Connect(~server.host, ~server.port, ~account.user, ~account.password)) {
...
void FtpBrowser::Browse()
{
	...
	if(user.IsEmpty())
		user = "anonymous";
	Title(Format(t_("Ftp Browser - %s@%s [%s]"), user, host, workdir));
	list.Clear();
	if(workdir != rootdir) {
		list.Add(t_("Go to Parent Directory"), CtrlImg::DirUp(), StdFont(), SColorText(), true,
	...
	for(int i = 0; i < dirlist.GetCount(); i++) {
		...
		list.Add(e.GetName(), e.IsFile() ? CtrlImg::File() : CtrlImg::Dir(), StdFont(),
		         SColorText(), e.IsDirectory(), e.GetSize(), e.GetLastModified(), Null, Null,
		         Null, RawPickToValue(pick(e)));
	}
}
...
void FtpBrowser::Download(const String& file, const String& path)
{
	...
	progress.Reset();
	progress.Create();
	int rc = FtpGet(remotefile, localfile, ~server.host, 21, ~account.user, ~account.password,
	                THISBACK(DownloadProgress), THISBACK(UpdateGui), true,
	                server.active, server.ftps);
	if(rc != 0)
...
{
	progress.Set((int)done, (int)total);
...
void FtpBrowser::Settings() { settings.Execute(); }

void FtpBrowser::About()
{
	PromptOK(t_("[A3 Simple Ftp Browser Example]&Using [*^www://upp.sf.net^ Ultimate`+`+] "
	            "technology."));
}
...
void FtpBrowser::HelpMenu(Bar& bar) { bar.Add(t_("About..."), THISBACK(About)); }
...
{
	Title(t_("Ftp Browser"));
	CtrlLayout(server);
	CtrlLayout(account);
	server.port.SetData(21);
