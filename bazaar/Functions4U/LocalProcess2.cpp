...
bool LocalProcess2::DoStart(const char *_command, const Vector<String> *arg, bool spliterr, const char *envptr, const char *dir)
{
	...
	std::wstring stdwsdir = wsdir.ToStd();
	const wchar_t *wdir;
	if (dir == 0)
		wdir = 0;
	else 
		wdir = stdwsdir.c_str();
	bool h = CreateProcessW(NULL, const_cast<wchar_t*>(wscmd.ToStd().c_str()), &sa, &sa, TRUE, NORMAL_PRIORITY_CLASS, (void *)envptr, wdir, &si, &pi);
