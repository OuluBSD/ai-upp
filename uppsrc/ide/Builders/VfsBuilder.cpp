...
void VfsBuilder::RunPkgConfig(Vector<String>& v)
{
	for(int i = 0; i < v.GetCount(); i++) {
		if (v[i] == "`pkg-config") {
			if (i+2 < v.GetCount() && v[i+2].Right(1) == "`") {
				String r = v[i+2];
				String cmd = v[i].Mid(1) + " " + v[i+1] + " " + r.Left(r.GetCount()-1);
				
				String out;
				int j = pkgconfig_cache.Find(cmd);
				if (j >= 0)
					out = pkgconfig_cache[j];
				else {
					Sys(cmd, out);
					pkgconfig_cache.Add(cmd, out);
				}
				
				Vector<String> parts = Split(out, " ");
				v.Remove(i, 3);
				v.Insert(i, parts);
			}
		}
		else if (TrimBoth(v[i]).IsEmpty())
			v.Remove(i--);
	}
}

bool VfsBuilder::RunFile(String cmdline, int slot, String file, String key, int blitz_count) {
	StringStream ss;
	bool b = RunFile(cmdline, ss, slot, file, key, blitz_count);
	String s = TrimBoth(ss.GetResult());
	if (!s.IsEmpty())
		PutConsole(s);
	return b;
}

bool VfsBuilder::RunFile(String cmdline, Stream& out, int slot, String filename, String key, int blitz_count) {
	Vector<String> args;
	...
	if (internal.IsEmpty()) {
		for (auto inc : Split(GetClangInternalIncludes(), ";")) {
			internal.Add("-I");
			internal.Add(inc);
		}
	}
	args.Insert(0, internal);
	
	for(int i = 0; i < args.GetCount(); i++) {
		String a = TrimBoth(args[i]);
		String a2 = a.Left(2);
		if (a.IsEmpty() || a == "-c" || a2 == "-g" || a2 == "-O")
			args.Remove(i--);
		else if (a2 == "-I" && a.GetCount() > 2) {
			args[i] = "-I";
			args.Insert(i+1, TrimBoth(a.Mid(2)));
		}
		else if (a[0] == '\"' && a.GetCount() >= 2 && a[a.GetCount()-1] == '\"')
			args[i] = a.Mid(1, a.GetCount()-2);
	}
	//DUMPC(args);
	...
	dword options =		CXTranslationUnit_KeepGoing|
	...
	CXErrorCode err = clang_parseTranslationUnit2(index, filename, argv, argv.GetCount(), 0, 0, options, &tu);
	
	if(err != CXError_Success || !tu) {
		PutConsole("Failed commandline: " + (String)cmdline);
		PutConsole("Errorcode: " + IntStr(err));
...
String VfsBuilder::CompilerName() const
{
	if(!IsNull(compiler)) return compiler;
	return "c++";
}

bool VfsBuilder::IsInternalCompiler() const
{
	return true;
}

String VfsBuilder::CmdLine(const String& package, const Package& pkg)
{
...
void VfsBuilder::CToAST(String fo, String objfile, const String& package, const Package& pkg)
{
	String tmpfile = ForceExt(objfile, ".c");
	SaveFile(tmpfile, fo);
	String cc = CmdLine(package, pkg);
	//cc << " -c -o " << GetPathQ(objfile) << " -x c " << GetPathQ(tmpfile);
	cc << " -x c "/*" -c -x c " << GetPathQ(tmpfile)*/;
	int slot = AllocSlot();
	if(slot < 0 || !RunFile(cc, slot, tmpfile, objfile, 1))
		throw Exc(Format("Error compiling binary object '%s'.", objfile));
}

bool VfsBuilder::BuildPackage(const String& package, Vector<String>& linkfile, Vector<String>& immfile,
	...
	int opt)
{
	...
	includes = GetPackagePathNest(packagedir);
	for (auto& i : pkg.include) {
		if (!i.text.IsEmpty()) {
			if (!includes.IsEmpty())
				includes.Cat(';');
			includes.Cat(i.text);
...
//	}
	...
	if(blitz && b.build) {
		...
		int slot = AllocSlot();
		if(slot < 0 || !RunFile(String().Cat() << Join(cc, cpp_options) << ' '
		                    << GetPathQ(b.path)
		                    /*<< " -o " << GetPathQ(b.object)*/, slot, b.path, b.object, b.count))
			error = true;
	}
	...
	for(i = 0; i < sfile.GetCount(); i++) {
		...
		if(HdependFileTime(fn) > GetFileTime(objfile)) {
			...
			if(rc) {
				...
				PutVerbose(exec);
				int slot = AllocSlot();
				execerr = (slot < 0 || !RunFile(exec, slot, objfile, 1));
#endif
			}
			else
			if(brc) {
				try {
					...
					CToAST(BrcToC(parser, GetFileDirectory(fn)), objfile, package, pkg);
				}
				catch(Exc e) {
					PutConsole(e);
					execerr = true;
				...
				PutVerbose(exec);
				int slot = AllocSlot();
				execerr = (slot < 0 || !RunFile(exec, slot, fn, objfile, 1));
			}
			if(execerr)
				DeleteFile(objfile);
			error |= execerr;
...
#if 0
bool VfsBuilder::CreateLib(const String& product, const Vector<String>& obj,
...
#endif
bool VfsBuilder::Link(const Vector<String>& linkfile, const String& linkoptions, bool createmap)
{
	if(!Wait())
		return false;
	PutLinking();
...
bool VfsBuilder::Preprocess(const String& package, const String& file, const String& target, bool asmout)
{
	Package pkg;
	pkg.Load(PackageFile(package));
	String packageDir = PackageDirectory(package);
...
Builder *CreateVfsBuilder()
{
	return new VfsBuilder;
}

INITIALIZER(VfsBuilder)
{
