...
void RunVirtualGui3D(VirtualGui3D& gui, Event<> app_main)
{
	VirtualGui3DPtr = &gui;
	Surface::InitFB();
	Surface::SetDesktopSize(gui.GetSize());
	app_main();
	Surface::ExitFB();
}
...
dword AtomVirtualGui3D::GetModKeys()
{
	return modkeys;
}

bool AtomVirtualGui3D::Poll(UPP::CtrlEvent& e) {
	TODO
}

void AtomVirtualGui3D::SetTitle(String title) {
	TODO
}

uint32 AtomVirtualGui3D::GetTickCount() {
	TODO
}

#if 0
void AtomVirtualGui3D::HandleSDLEvent(SDL_Event* event)
{
...
void AtomVirtualGui3D::WaitEvent(int ms)
{
	Panic("not supported");
	//SDL_WaitEventTimeout(NULL, ms);
}

bool AtomVirtualGui3D::IsWaitingEvent()
{
	Panic("TODO");
	return false;
	/*SDL_PumpEvents();
...
void AtomVirtualGui3D::WakeUpGuiThread()
{
	Panic("TODO");
