...
{ std::memset(t, x, size_t(lim - t)); }
inline void Copy(char *dst, const char *src, const char *lim)
{ std::memcpy(dst, src, size_t(lim - src)); }
...
{
	std::memcpy(reinterpret_cast<void *>(dst), reinterpret_cast<void *>(src), sizeof(T));
}
...
struct RelocateTrivial<T,true>
{
	static void Insert(T *dst, T *src, int n) {
		std::memmove(reinterpret_cast<void *>(dst), reinterpret_cast<void *>(src), n * sizeof(T));
	}
	static void Remove(T *dst, T *src, int n) {
		std::memmove(reinterpret_cast<void *>(dst), reinterpret_cast<void *>(src), n * sizeof(T));
	}
	static void Relocate(T *dst, T *src, int n) {
		std::memcpy(reinterpret_cast<void *>(dst), reinterpret_cast<const void *>(src), n * sizeof(T));
	}
};
...
public:
	...
	ConstIIterator()          {}
	ConstIIterator(NP *null)  { ASSERT(null == nullptr); ii = -1; }
...
public:
	...
	IIterator()          {}
	IIterator(NP *null)  { ASSERT(null == nullptr); ii = -1; }
