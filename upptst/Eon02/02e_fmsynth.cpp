...
void Run02eFmsynth(Engine& eng, int method) {
	...
	case 3: {
		...
		auto& pipe = input.AddAtom("pipe0", "fmsynth.pipe");
		pipe.args.GetAdd("loop") = String("event");
		pipe.args.GetAdd("filepath") = ShareDirFile("presets/fmsynth/presets/old_school_organ.fmp");
		...
		nets.Add(&event);
		nets.Add(&input);
		if (!BuildRouterChain(eng, nets, "RouterNetContext: midi.app.program fmsynth nets link event/input loops."))
			Exit(1);
		break;
	}
	case 1:
	...
	case 4:
		sys->PostLoadPythonFile(ShareDirFile("py/eon/02e_fmsynth_method4.py"));
		return;
	case 0:
	default:
