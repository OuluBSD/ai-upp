...
# Ensure BaseFlags is not null
if ($null -eq $BaseFlags) {
    $BaseFlags = ""
}
...
if ($finalFlags) {
    $buildModel = "C:\Users\$env:USERNAME\upp\ANDROID.bm"
    $command = "umk ./examples,./upptst,./uppsrc `"$Target`" `"$buildModel`" `"$buildFlags`" `"+$finalFlags`" `"bin/$Target.exe`""
} else {
    $buildModel = "C:\Users\$env:USERNAME\upp\ANDROID.bm"
    $command = "umk ./examples,./upptst,./uppsrc `"$Target`" `"$buildModel`" `"$buildFlags`" `"bin/$Target.exe`""
}
