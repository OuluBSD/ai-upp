...
void Geometry4::DrawLine(DrawGeometry& fb, vec2 a, vec2 b, Color clr) {
	a = (view * a.Embed<4>()).Project().Splice<0,2>();
	b = (view * b.Embed<4>()).Project().Splice<0,2>();
	fb.DrawLine(a, b, 1, clr);
}

void Geometry4::ResetShape(Shape2DWrapper& s, ShapeId shape_type, bool is_right) {
	vec2 a {0, 0}, b {0, 0};
	float f = 0;
...
void Geometry4::Initialize() {
	...
	for(int i = 0; i < 5; i++) {
		...
		Tutorial4Dummy& d = *c->Add<Tutorial4Dummy>();
		PrimitiveShape& s = *c->Add<PrimitiveShape>();
		...
		// Set random speed
		b.velocity = vec3(Randomf(), Randomf());
		
		// Set random turning speed
		d.angle = Randomf() * (0.2 * M_2PI);
		
		// Set primitives
		vec2 shape_vec {
			0.1f + (float)Randomf() * 0.9f,
			0.1f + (float)Randomf() * 0.9f};
		s.SetShape(GetRandomShape2(), shape_vec);
		s.Dispatch();
	}
