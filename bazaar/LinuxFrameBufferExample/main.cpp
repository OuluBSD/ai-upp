...
#include <LinuxFrameBuffer/LinuxFrameBuffer.h>

using namespace Upp;

#define IMAGECLASS UWordImg
#define IMAGEFILE  <LinuxFrameBufferExample/UWord.iml>
#include <Draw/iml.h>
...
{
	AppInit__(argc, (const char **)argv, (const char **)envp);
	StdLogSetup(LOG_FILE | LOG_COUT | LOG_TIMESTAMP);
	
	LinuxFrameBuffer gui;
	if (!gui.Create("/dev/fb0")){
		return 1;
	}
		
	RunVirtualGui(gui, AppMain);
