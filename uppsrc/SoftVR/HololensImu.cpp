 ...
 */
static int hololens_imu_send_command(OuvrtDevice *dev, uint8 command)
{
	uint8 data[64] = { 0x02, command };
	int ret;
					  ...
					  struct hololens_imu_report *report)
{
	...
	for (int i = 0; i < 4; i++) {
		...
		raw.acc[0] = (int32)__le32_to_cpu(report->accel[0][i]);
		raw.acc[1] = (int32)__le32_to_cpu(report->accel[1][i]);
		raw.acc[2] = (int32)__le32_to_cpu(report->accel[2][i]);
		/* Angular velocity in 10⁻³ rad/s @ 8 kHz */
		raw.gyro[0] = (int16)__le16_to_cpu(report->gyro[0][8 * i + 0]) +
			      (int16)__le16_to_cpu(report->gyro[0][8 * i + 1]) +
			      (int16)__le16_to_cpu(report->gyro[0][8 * i + 2]) +
			      (int16)__le16_to_cpu(report->gyro[0][8 * i + 3]) +
			      (int16)__le16_to_cpu(report->gyro[0][8 * i + 4]) +
			      (int16)__le16_to_cpu(report->gyro[0][8 * i + 5]) +
			      (int16)__le16_to_cpu(report->gyro[0][8 * i + 6]) +
			      (int16)__le16_to_cpu(report->gyro[0][8 * i + 7]),
		raw.gyro[1] = (int16)__le16_to_cpu(report->gyro[1][8 * i + 0]) +
			      (int16)__le16_to_cpu(report->gyro[1][8 * i + 1]) +
			      (int16)__le16_to_cpu(report->gyro[1][8 * i + 2]) +
			      (int16)__le16_to_cpu(report->gyro[1][8 * i + 3]) +
			      (int16)__le16_to_cpu(report->gyro[1][8 * i + 4]) +
			      (int16)__le16_to_cpu(report->gyro[1][8 * i + 5]) +
			      (int16)__le16_to_cpu(report->gyro[1][8 * i + 6]) +
			      (int16)__le16_to_cpu(report->gyro[1][8 * i + 7]);
		raw.gyro[2] = (int16)__le16_to_cpu(report->gyro[2][8 * i + 0]) +
			      (int16)__le16_to_cpu(report->gyro[2][8 * i + 1]) +
			      (int16)__le16_to_cpu(report->gyro[2][8 * i + 2]) +
			      (int16)__le16_to_cpu(report->gyro[2][8 * i + 3]) +
			      (int16)__le16_to_cpu(report->gyro[2][8 * i + 4]) +
			      (int16)__le16_to_cpu(report->gyro[2][8 * i + 5]) +
			      (int16)__le16_to_cpu(report->gyro[2][8 * i + 6]) +
			      (int16)__le16_to_cpu(report->gyro[2][8 * i + 7]);
		 ...
		 *
		 */
		imu.acceleration[0] = raw.acc[1] * -1e-3;
		imu.acceleration[1] = raw.acc[0] * -1e-3;
		imu.acceleration[2] = raw.acc[2] * -1e-3;
		imu.angular_velocity[0] = raw.gyro[1] * -(1e-3 / 8.0);
		imu.angular_velocity[1] = raw.gyro[0] * -(1e-3 / 8.0);
		imu.angular_velocity[2] = raw.gyro[2] * -(1e-3 / 8.0);
 ...
 * for an answer.
 */
static int hololens_imu_command_sync(OuvrtDevice *dev, uint8 command,
				     union hololens_report *report)
{
	int ret;
 ...
 */
static int hololens_imu_config_read(OuvrtDevice *dev, uint8 command,
				    uint8 *buf, size_t count)
{
	union hololens_report report;
...
static int hololens_imu_start(OuvrtDevice *dev)
{
	...
	config = (uint8*)calloc(config_len, 1);
	if (!config)
		return -ENOMEM;
...
static void hololens_imu_thread(OuvrtDevice *dev)
{
	...
	while (dev->active) {
		...
		if (ret == HOLOLENS_IMU_REPORT_SIZE_V2 &&
			...
			hololens_imu_handle_imu_report(self, (struct hololens_imu_report *)buf);
		} else if (ret == HOLOLENS_IMU_REPORT_SIZE &&
		    buf[0] == HOLOLENS_IMU_REPORT_ID) {
			hololens_imu_handle_imu_report(self, (struct hololens_imu_report *)buf);
		} else if (ret == HOLOLENS_CONTROL_REPORT_SIZE &&
			   buf[0] == HOLOLENS_CONTROL_REPORT_ID) {
			hololens_imu_handle_control_report(self, (struct hololens_control_report *)buf);
		} else if (ret == HOLOLENS_DEBUG_REPORT_SIZE &&
			   buf[0] == HOLOLENS_DEBUG_REPORT_ID) {
			hololens_imu_handle_debug_report(&self->dev, (struct hololens_debug_report *)buf);
		} else {
