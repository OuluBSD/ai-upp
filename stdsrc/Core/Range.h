...
#include <vector>
#include "Core.h"
#include "Util.h" // for ASSERT and other utilities
...
public:
    using value_type = typename std::remove_reference<decltype(*l)>::type;
...
struct ConstRangeClass {
    ...
    using Iterator = ConstIIterator<ConstRangeClass>;
...
struct ReverseRangeClass {
    ...
    using value_type = ValueTypeOf<BaseRange>;
    using ValueType = value_type;
    ...
    using Iterator = IIterator<ReverseRangeClass>;
    using ConstIterator = ConstIIterator<ReverseRangeClass>;
...
struct ViewRangeClass {
    ...
    int GetCount() const                      { return ndx.size(); }

    using Iterator = IIterator<ViewRangeClass>;
    using ConstIterator = ConstIIterator<ViewRangeClass>;

    ViewRangeClass& Write()                   { return *this; }

    ConstIterator begin() const { return ConstIterator(*this, 0); }
    ConstIterator end() const  { return ConstIterator(*this, ndx.size()); }

    Iterator begin() { return Iterator(*this, 0); }
    Iterator end()   { return Iterator(*this, ndx.size()); }
    ...
    ViewRangeClass(BaseRange& r, std::vector<int>&& ndx) : r(&r), ndx(std::move(ndx)) {}
    ViewRangeClass() {} // MSC bug workaround
};

template <class BaseRange>
ViewRangeClass<BaseRange> ViewRange(BaseRange&& r, std::vector<int>&& ndx)
{
    return ViewRangeClass<BaseRange>(r, std::move(ndx));
}
