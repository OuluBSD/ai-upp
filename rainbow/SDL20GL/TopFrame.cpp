...
{
	close.SetImage(SDL20GLFBImg::close());
	close.EdgeStyle();
	Add(close);
	maximize.SetImage(SDL20GLFBImg::maximize());
	maximize.EdgeStyle();
	Add(maximize);
...
void TopWindowFrame::Maximize()
{
	if(!maximized && maximize.IsShown()) {
		...
		maximize.SetImage(SDL20GLFBImg::overlap());
	}
}
...
{
	return maximized ? Rect(0, 0, 0, 0) : ChMargins(SDL20GLFBImg::border());
}
...
void TopWindowFrame::Paint(Draw& w)
{
	...
	ChPaintEdge(w, sz, SDL20GLFBImg::border());
	ChPaint(w, m.left, m.top, sz.cx - m.left - m.right, GetStdFontCy() + 4,
	        window->IsForeground() ? SDL20GLFBImg::title() : SDL20GLFBImg::bgtitle());
	int tx = m.left + 2;
...
{
	Size sz = GetSize();
	Rect m = ChMargins(SDL20GLFBImg::border());
	Point dir;
