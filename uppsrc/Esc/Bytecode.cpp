...
void EscCompiler::OutOfMemory()
{
	ThrowError("Out of memory");
}

void EscCompiler::TestLimit()
{
	LTIMING("TestLimit2");
...
void EscCompiler::Assign(const IrValue& src)
{
	Emit1(IR_ASSIGN_R_LVAL_VALUE, src);
}

void EscCompiler::Subscript(String id)
{
	LTIMING("Subscript");
...
void EscCompiler::Subscript()
{
	Subscript(String());
}

void EscCompiler::Term()
{
	Emit(IR_R_SBS_RESET);
...
void EscCompiler::DoCompare(const char *op)
{
	Emit(IR_PUSH_R_EMPTY);
	Shift();
	Emit(IR_POP_R_TEMP);
...
int EscCompiler::stack_level = 50;

void EscCompiler::Exp()
{
	LTIMING("Exp");
	Spaces();
	stack_level--;
...
void EscCompiler::SwapIR(Vector<IR>& ir) {
	Swap(ir, this->ir);
}

void EscCompiler::WriteLambda(EscLambda& l) {
	Swap(l.ir, ir);
	l.compiled = true;
}

void EscCompiler::Emit_(IrCode x, const char* file, int line) {
	IR& ir = this->ir.Add();
	ir.code = x;
	ir.file = file;
	ir.line = line;
...
void EscCompiler::Emit1_(IrCode x, IrValue a, const char* file, int line) {
	IR& ir = this->ir.Add();
	ir.code = x;
	ir.file = file;
	ir.line = line;
...
void EscCompiler::Emit2_(IrCode x, IrValue a, IrValue b, const char* file, int line) {
	IR& ir = this->ir.Add();
	ir.code = x;
	ir.file = file;
	ir.line = line;
...
void EscCompiler::Emit3_(IrCode x, IrValue a, IrValue b, IrValue c, const char* file, int line) {
	IR& ir = this->ir.Add();
	ir.code = x;
	ir.file = file;
	ir.line = line;
...
void EscCompiler::EmitLabel_(IrValue l, const char* file, int line) {
	IR& ir = this->ir.Add();
	ir.code = IR_LABEL;
	ir.file = file;
	ir.line = line;
...
IrValue EscCompiler::EmitSelfLambdaCheck(String id, IrValue& tmp, const char* file, int line) {
	IR& ir = this->ir.Add();
...
IrValue EscCompiler::EmitGlobalLambdaCheck(String id, IrValue& tmp, const char* file, int line) {
	IR& ir = this->ir.Add();
...
IrValue EscCompiler::EmitSelfLvalCheck(const char* file, int line) {
	IR& ir = this->ir.Add();
...
IrValue EscCompiler::EmitPushVar_(const IrValue& v, const char* file, int line) {
	IR& ir = this->ir.Add();
	ir.code = IR_PUSH_VAR;
	ir.file = file;
	ir.line = line;
...
IrValue EscCompiler::EmitPopVar_(const IrValue& v, int reg, const char* file, int line) {
...
IrValue EscCompiler::EmitPopVar_(const IrValue& v, const IrValue& avoid0, const char* file, int line) {
	ASSERT(v.IsVarStackValue(0));
...
IrValue EscCompiler::CreateLabel() {
	return IrValue().SetLabel(lbl_counter++);
}

void EscCompiler::PushLoop(IrValue exit) {
	LoopStack& l = loop_stack.Add();
	l.exit = exit;
	loop++;
}

void EscCompiler::PushLoop(IrValue exit, IrValue repeat) {
	LoopStack& l = loop_stack.Add();
	l.exit = exit;
	l.repeat = repeat;
	loop++;
...
void EscCompiler::CreateSwitchDefault() {
	TODO
}

void EscCompiler::PopLoop() {
	loop_stack.Remove(loop_stack.GetCount()-1);
	loop--;
}

IrValue EscCompiler::IsTrue(const IrValue& v) {
	Emit1(IR_R_ISTRUE, v);
	IrValue r;
...
void EscCompiler::OnError(String msg) {
	fail = true;
